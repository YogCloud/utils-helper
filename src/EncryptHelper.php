...
class EncryptHelper
{
    ...
    public static function authcode(string $data, string $key, bool $encode = true, int $expiry = 0): array
    {
        ...
        // 密钥c用于变化生成的密文
        $keyc = $encode ? substr(md5(microtime()), -self::DYNAMIC_KEY_LEN) : substr($data, 0, self::DYNAMIC_KEY_LEN);
        $keyd = md5($keya . $keyc);
        $cryptkey = $keya . $keyd;
        ...
        if ($encode) {
            ...
            $data = self::base64UrlDecode(substr($data, self::DYNAMIC_KEY_LEN));
        }
    ...
    public static function easyEncrypt(string $data, string $key): string
    {
        ...
        return substr($key, 0, self::DYNAMIC_KEY_LEN) . self::base64UrlEncode($str);
    }
    ...
    {
        if (strlen($data) < self::DYNAMIC_KEY_LEN) {
            return '';
        }

        $key = md5($key);
        if (strpos($data, substr($key, 0, self::DYNAMIC_KEY_LEN)) !== 0) {
            return '';
        }
